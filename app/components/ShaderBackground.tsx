...
const fragmentShader = `
  ...
  void main() {
    ...
    // ============ FLUID TRAIL EFFECT (Wake behind cursor) ============
    if (uFluidStrength != 0.0) {
      ...
      // Smooth gaussian falloff - creates soft, fluid-like influence area
      float influence = exp(-dist * dist / (uFluidRadius * uFluidRadius));
      influence = pow(influence, uFluidSoftness);
      
      // Get mouse velocity (normalized direction of movement)
      vec2 velocity = uMouseVelocity;
      float speed = length(velocity);
      
      // Displacement is in the direction of mouse movement
      // The gradient gets "pushed" by the cursor as it moves
      vec2 displacement = velocity * influence * uFluidStrength * 0.5;
      
      // Correct aspect ratio
      displacement.x /= ratio;
      
      // Apply the fluid warp - push UVs in the direction of movement
      tUv -= displacement;
    }
...
function GradientPlane() {
  ...
  // Fluid trail controls
  const fluid = useControls("🌊 Fluid Trail", {
    fluidStrength: {
      value: 1.5,
      min: 0.0,
      max: 5.0,
  ...
  const uniforms = useMemo(
    () => ({
      ...
      // Fluid trail uniforms
      uFluidStrength: { value: fluid.fluidStrength },
      uFluidRadius: { value: fluid.fluidRadius },
      uFluidSoftness: { value: fluid.fluidSoftness },
  ...
  useFrame((state) => {
    if (materialRef.current) {
      ...
      // Calculate mouse velocity with smoothing (lerp for fluid motion)
      const smoothing = fluid.fluidSmoothing;
      const targetVx = (mouseRef.current.x - mouseRef.current.prevX) * 10;
      const targetVy = (mouseRef.current.y - mouseRef.current.prevY) * 10;

      // Smooth interpolation for fluid-like trailing effect
      mouseRef.current.vx += (targetVx - mouseRef.current.vx) * smoothing;
      mouseRef.current.vy += (targetVy - mouseRef.current.vy) * smoothing;

      // Store current position as previous for next frame
      mouseRef.current.prevX = mouseRef.current.x;
      mouseRef.current.prevY = mouseRef.current.y;

      // Update fluid trail parameters
      materialRef.current.uniforms.uFluidStrength.value = fluid.fluidStrength;
      materialRef.current.uniforms.uFluidRadius.value = fluid.fluidRadius;
      materialRef.current.uniforms.uFluidSoftness.value = fluid.fluidSoftness;
